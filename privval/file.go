...
func (pv *FilePV) SigCrossMerkleRoot(MerkleRoot []byte, Blockhash []byte, vote *types.Vote) error {

	if signature, err := pv.Key.PrivKey.Sign(Blockhash); err != nil {
		return nil
	} else {
		vote.PartBlockSig.PeerCrossSig = signature
	}
